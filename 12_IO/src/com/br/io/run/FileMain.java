...
public class FileMain {
	public static void main(String[] args) {
		...
		// 1. 경로 지정하지 않고 파일명 제시해서 생성 => 현재 project폴더 내에 생성
		try {
		File file1 = new File("test.txt");
		file1.createNewFile();
		System.out.println("test.txt 파일이 성공적으로 만들어졌습니다");
...
//			file2.createNewFile();
//			System.out.println("D드라이브내에 test.txt파일이 성공적으로 만들어졌습니다.");
//			// *존재하지 않는 디렉토리 경로 제시시 => IOException
...
//			System.out.println("D:\\temp 내에 test.txt파일이 성공적으로 만들어졌습니다.");
			...
			System.out.println("파일용량 : "+file3.length());//byte단위
			System.out.println("상위폴더 : "+file3.getParent());
			System.out.println("파일이냐 : "+file3.isFile());
			System.out.println("파일이냐 : "+file3.isDirectory());
		 ...
		 * < 스트림 >
		 * 
		 * 1. 자바프로그램과 외부매체와 (키보드 , 모니터 , 파일 , 프로그램)입출력을 하기 위한 통로같은 개념
		 * 	  입출력을 하기 위한 통로같은 개념 
		 * 2. 단반향의 특징을 가지고 있음 (입력용 스트림, 출력용 스트림)
		 * 3. 선입선출(FIFO)의 특징을 가지고 있음 => 시간지연이 발생될 수 있음
		 * 4. 구분
		 * 	  1)통로의 사이즈 (1byte짜리 / 2byte짜리)
		 * 		ㄴ 바이트스트림 : 1byte짜리만 입출력할 수 있는 좁은통로		=> 입력(InputStream)	|	출력(OutputStream)
		 * 		ㄴ 문자스트림	  : 2byte짜리까지도 입출력할 수 있는 넓은통로		=> 입력(Reader)		|	출력(Writer)
		 * 
		 * 	  2)외부매체와 직접 연결 유무 
