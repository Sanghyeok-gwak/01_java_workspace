...
public class LibraryManager {
	...
	public List<Book> insertBook(int num , String title, String author 
								, String publisher ,boolean coupon,int accessAge ){
		if(num==1) {
			bookList.add(new AniBook(title,author,publisher,accessAge));
		}
		else if(num==2) {
			bookList.add(new CookBook(title,author,publisher,coupon));
		}
	...
	public int updateBook(int index , String title, String author 
							, String publisher ,boolean coupon,int accessAge) {
		int result=-1;
		for(int i =0; i<bookList.size(); i++) {
			if(index == i) {
				if(bookList.get(i) instanceof CookBook ) {
					((CookBook)bookList.get(i)).setTitle(title);
					((CookBook)bookList.get(i)).setAuthor(author);
					((CookBook)bookList.get(i)).setPublisher(publisher);
					((CookBook)bookList.get(i)).setCoupon(coupon);
				}
				else if(bookList.get(i) instanceof AniBook) {
					((AniBook)bookList.get(i)).setTitle(title);
					((AniBook)bookList.get(i)).setAuthor(author);
					((AniBook)bookList.get(i)).setPublisher(publisher);
					((AniBook)bookList.get(i)).setAccessAge(accessAge);
				}
				result =1;
			}
